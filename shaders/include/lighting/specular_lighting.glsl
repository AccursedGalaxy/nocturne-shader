...
vec3 get_specular_reflections(
    ...
    // Nocturne: coherent march for the mirror path (constant mid-step
    // offset). Per-pixel dither decorrelates neighboring rays and turns
    // the hit/miss boundary at thin geometry (campfires, fences) into
    // hard speckle on metals; bench A/B (grain-A scene) showed even
    // quarter-amplitude dither brings it back. The residual is a small
    // coherent band zone beside emissives - strictly less objectionable.
